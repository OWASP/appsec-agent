...
- **`resolveProvider()`** — Returns `ClaudeProvider`, `CodexProvider`, or `DeepInfraProvider` from `AGENT_PROVIDER`.
...
- DeepInfra: `roleSpecToDeepInfraClientOptions()` + a local agent loop over the `openai` SDK, with local `Read`/`Grep`/`Write`/`Bash` tool executors (`src/tools/`) and an MCP bridge (`src/providers/deepinfra_mcp_bridge.ts`)

Structured JSON outputs are validated in `src/providers/structured_output.ts` (Codex and DeepInfra paths).
