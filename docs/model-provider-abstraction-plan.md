...
>
> Update (v4.0.0, breaking): the third provider was reworked from Moonshot's direct API to **DeepInfra** — a HIPAA/SOC 2-certified inference cloud hosting open-weight models (Kimi, DeepSeek, GLM, Qwen, gpt-oss) behind the same OpenAI-compatible surface. Since DeepInfra's chat-completions API is wire-compatible with what the Moonshot provider already implemented, this was primarily a rename (`moonshot_*` → `deepinfra_*` modules, `--provider moonshot` → `--provider deepinfra`, `MOONSHOT_API_KEY` → `DEEPINFRA_API_KEY`) plus three substantive changes: model ids became vendor-namespaced slugs (`moonshotai/Kimi-K2.6`) with a short-alias layer for ergonomics; cost switched from a hand-maintained per-model price table to DeepInfra's exact per-request `estimated_cost`, falling back to live pricing from `GET /v1/models`; and a `DEEPINFRA_REASONING_EFFORT` knob (default `medium`) was added because DeepInfra's reasoning models otherwise reason unboundedly on every turn. No Moonshot compatibility shim was kept — see the `4.0.0` [CHANGELOG](../CHANGELOG.md) entry for the migration path.
