...
function buildContextExtractorPrompt(ctx: ExtractionContext): string {
  ...
  parts.push('5. **suggested_exclusions**: Based on the repository tree structure, suggest glob patterns for directories/files that should be EXCLUDED from security scanning. Only suggest project-specific patterns not already in the standard preset (which covers: node_modules, vendor, .git, __pycache__, dist, build, tests, __tests__, e2e, spec, helm, terraform, .codefresh, .next, test files like *.test.ts, *.spec.ts, *.stories.ts). Look for: generated code, vendored third-party copies, large asset directories (images, fonts), documentation-only dirs, migration/seed scripts, example/sample code, IDE config, visual assets. Return as comma-separated glob patterns (e.g., "**/migrations/**, **/visual_dictionary/**, **/seed/**"). Return empty string if no project-specific exclusions are needed.');
